...
def data_import():
    books = pd.read_csv('https://raw.githubusercontent.com/zygmuntz/\
        goodbooks-10k/master/books.csv')
    ratings = pd.read_csv('https://raw.githubusercontent.com/zygmuntz/\
        goodbooks-10k/master/ratings.csv')
...
def create_matrix(books, ratings):
    ...
    matrix = df.pivot(
        index='title', columns='user_id', values='rating').fillna(0)
    return df
